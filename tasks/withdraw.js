...
task('withdraw', 'withdraw token from pool')
  ...
  .addOptionalParam("account", "The receiver", 0,types.int)
  .setAction(async ({ token, amount,account }) => {
    ...
    let oldBalance = await erc20.balanceOf(accounts[account].address);
    oldBalance = new BigNumber(oldBalance+'').shiftedBy(-decimals).toString(10);
    ...
    await (await lendingPool.withdraw(reserve,amountOx,accounts[account].address,{gasLimit:5000000})).wait(1);
    console.log(`withdraw reserve: [${reserve}] amount = ${amount} ${symbol} for account: ${accounts[account].address}`);

    let newBalance = await erc20.balanceOf(accounts[account].address);
    newBalance = new BigNumber(newBalance+'').shiftedBy(-decimals).toString(10);

    console.log(`oldBalance = ${oldBalance} ${symbol}, newBalance = ${newBalance} ${symbol}, withdraw ${new BigNumber(newBalance).minus(oldBalance).toString(10)} ${symbol} actually`);
